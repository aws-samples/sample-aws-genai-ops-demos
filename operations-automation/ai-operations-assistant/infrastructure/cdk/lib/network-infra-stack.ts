        ...
        bundling: {
          ...
          local: {
            tryBundle(outputDir: string) {
              ...
              // nosemgrep: detect-child-process — outputDir is a CDK-internal temp path
              // generated by AssetStaging, not user input. This runs at synth/deploy time
              ...
              try {
                execSync(
                  `pip install --no-cache-dir --platform manylinux2014_x86_64 --implementation cp --python-version 3.12 --only-binary=:all: --target "${outputDir}" -r requirements.txt`, // nosemgrep: detect-child-process — outputDir is CDK-internal (AssetStaging), not user input
                  { cwd: NETWORK_TRANSFORMATION_LAMBDA_DIR, stdio: 'inherit' },
                );
                // Copy source files
                const fs = require('fs'); // nosemgrep: lazy-load-module — CDK synth-time code in tryBundle, not a request handler
                for (const f of fs.readdirSync(NETWORK_TRANSFORMATION_LAMBDA_DIR)) {
                  if (f.endsWith('.py') || f === 'requirements.txt') {
                    fs.copyFileSync(
                      path.join(NETWORK_TRANSFORMATION_LAMBDA_DIR, f), // nosemgrep: detect-non-literal-fs-filename, path-join-resolve-traversal — outputDir is CDK-internal
                      path.join(outputDir, f), // nosemgrep: detect-non-literal-fs-filename, path-join-resolve-traversal — outputDir is CDK-internal
                    );
                  }
                }
                return true;
