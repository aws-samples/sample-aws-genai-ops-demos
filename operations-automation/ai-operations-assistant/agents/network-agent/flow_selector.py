...
def query_matched_streams(
    ...
    sql = (
        "WITH matched AS ("
        "SELECT tcp_stream, src_ip, src_port, dst_ip, dst_port, "
        "MIN_BY(src_ip, frame_time) "
        "FILTER (WHERE "
        "(from_base(replace(tcp_flags, '0x', ''), 16) & 18) = 2"
        ") "
        "OVER (PARTITION BY tcp_stream) AS initiator_ip, "
        "MIN_BY(src_port, frame_time) "
        "FILTER (WHERE "
        "(from_base(replace(tcp_flags, '0x', ''), 16) & 18) = 2"
        ") "
        ...
        "MIN_BY(src_ip, frame_time) "
        "FILTER (WHERE "
        "(from_base(replace(tcp_flags, '0x', ''), 16) & 18) = 18"
        ") "
        ...
        "COALESCE(MAX(initiator_ip), MIN(src_ip)) AS client_ip, "
        "COALESCE(MAX(initiator_port), MIN(src_port)) AS client_port, "
        "COALESCE(MAX(responder_ip), MIN(dst_ip)) AS server_ip, "
        "COALESCE(MAX(responder_port), MIN(dst_port)) AS server_port, "
        "COUNT(*) AS packet_count "
        "FROM matched "
        "GROUP BY tcp_stream "
        "ORDER BY packet_count DESC, tcp_stream "
        "LIMIT 100"
    )
