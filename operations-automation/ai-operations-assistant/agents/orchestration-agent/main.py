...
# a concrete observation to anchor its sentence on.
#
...
def _hint_check_tls_hello_size(data: object) -> Optional[str]:
    """Report TLS Client Hello size statistics from captured data."""
    ...
    if over:
        parts.append(
            ...
            f"exceed {_TLS_FRAGMENTATION_THRESHOLD_BYTES} bytes"
        )
    if fragmented:
        parts.append(
            f"{fragmented} Client Hello{'s' if fragmented != 1 else ''} "
            "had fragment_count > 1"
        )
    if not parts:
        return None
    return "; ".join(parts) + "."
...
def _build_system_prompt() -> str:  # nosec B608 — f-string prompt template, not SQL
    ...
    # nosec B608 — f-string interpolates datetime values (today/current_year), not user input
...
- Includes a one-sentence interpretation seeded by a deterministic
  per-action hint based on the returned data.
...
- When ``anomalies`` contains ``tls_client_hello_fragmented``, the TLS
  Client Hello was split across multiple TCP segments. Investigate
  further to determine the impact on the specific network path.
