 ...
 * Reuse
 * -----
 * This construct is self-contained except for its Lambda handler, which lives at
 * ../../lambda/devops-agent-webhook-provisioner/ — copy both together when lifting
 * this into another project. Deploy it in a region where AWS DevOps Agent is
 * available (the containing stack's region).
 *
 * Requires aws-cdk-lib >= 2.267.0 (verified floor for AWS::DevOpsAgent::AgentSpace
 * with OperatorApp and the Aws association configuration).
 */
...
export class DevOpsAgentSpace extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DevOpsAgentSpaceProps) {
    ...
    // onboarding flow and the official CDK sample establish.
    ...
    // consistent. (Same class of race as the documented CfnService 403 in the
    // saas-status-mcp registration stack.)
    ...
    const provisionerFn = new lambda.Function(this, 'WebhookProvisioner', {
      ...
      // Pre-bundled by npm run bundle. @aws-sdk/client-devops-agent is included
      // because it is not guaranteed to exist in the Lambda runtime.
